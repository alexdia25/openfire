...
- **Applied in the port:** `game/pack.gd` loads a pack's `audio/audio.json` (section 2.4.2's schema, emitted by
  ...
  **The same cross-reference extends to tile-destroy effects and the mine explosion**: `coastal_destroy_effect_ids`
  ...
  32 of the 42 traced cues are wired to a real trigger as of 2026-09-22 (`FuelWarn` added last, a plain
  one-eighth-of-a-tank threshold in `Vehicle._process_fuel_warn()`; `PreRaise` just before that via the
  hangar confirm script's own sound steps, `game/selector_anim.gd`): the ten from the first two passes (the empty
