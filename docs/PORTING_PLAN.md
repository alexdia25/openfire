...
- **Applied in the port:** `game/pack.gd` loads a pack's `audio/audio.json` (section 2.4.2's schema, emitted by
  ...
  **The same cross-reference extends to tile-destroy effects and the mine explosion**: `coastal_destroy_effect_ids`
  ...
  33 of the 42 traced cues are wired to a real trigger as of 2026-09-22 (`JeepStart` added last, a real
  per-type "just created" sound at each vehicle-type record's `+0x240`, the same field that plays `Servo`
  for the Heli -- not yet reconciled with the already-wired `Heli` chime, see that cue's own note;
  `FuelWarn` just before that, a plain one-eighth-of-a-tank threshold in `Vehicle._process_fuel_warn()`;
  `PreRaise` before that via the hangar confirm script's own sound steps, `game/selector_anim.gd`): the
  ten from the first two passes (the empty
  ...
  tile-destroy and the mine explosion, the mine's own fuse-blink beep (`Button`, `Mine.beep` — already a real signal,
...
14. **Decorations (trees, at minimum) are never placed via the flat terrain tile grid —
    MECHANISM FOUND (2026-09-07), exact art id still open.** Chasing a user-reported "the
    reference footage's trees look real/camera-aware, not flat" observation found that no
    level in this game places any tree art at all via the ordinary tile grid: the one registry
    entry named `decoration.tree` (cel 101) was a misclassification (corrected to
    `terrain.structure.small_bunker` -- no canopy/foliage anywhere in the actual atlas crop),
    and real tree art (cels 135/138/139/140, unmistakable palm fronds and crossed coconut
    trunks) is never referenced by any of the 204 real levels' tile grids. Why: section 1.5/1.7
    already established that every raw tile byte resolves to a **0-127** art id
    (`*puVar4 & 0x7f`, confirmed straight from RFIRE.BIN, reproduced in `tools/rf_tile_art.py`)
    -- the real tree cels are all numbered above 127, so the flat terrain blitter is
    ...
    **Continuing the same thread found the actual mechanism (2026-09-07):** decompiling the
    ...
    word (bits 7-13, separate from the art-id field in bits 0-6). The terrain blitter's own
    per-tile helper (`FUN_00408c80`) reads that field back and, when nonzero, passes
    `tools/rf_tile_art.py`'s already-dumped `COASTAL_TABLE[id]["valid"]` field -- previously
    read only as a boolean "is this coastal id real" flag -- to `FUN_0041afb0` as a genuine
    object-type descriptor pointer, queuing a real object at that tile's exact centre through
    the *same* depth-sorted render queue vehicles use (confirmed by decompiling
    `FUN_0041afb0` itself: it reads position-offset fields and two callback pointers from that
    descriptor, exactly the "art/CCB pointer plus callback pointers" shape document 31 already
    found for the shared vehicle object-type table, and inserts the result into a depth-sorted
