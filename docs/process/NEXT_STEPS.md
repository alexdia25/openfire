...
- **A prototype confirming the "little triangles" complaint is a rendering-technique gap, not missing art** — a real, non-billboarded quad lying flat like the terrain plane, rotated in true 3D to match the vehicle's actual heading, produces a smooth, always-coherent silhouette at every heading using just **one** source image — better than the old nine-frame mirror-and-flip approach ever achieved, including at exactly the 90/270-degree headings that used to degrade to disconnected slivers. Toggle: `RF_DEBUG_VEHICLE_QUAD_MODE=ground_decal` on `game/vehicle_billboard_3d.gd` (default stays the verified Phase 3 billboard). See [document 32](32-worked-example-ground-decal-prototype.md); plan section 2.2, section 4 item 10.
