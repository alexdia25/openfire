...
- **The panel's scale and position in the `modern` layout** (3 px per original pixel, bottom-left; the `classic` layout is the traced original, document 96), the radar's black outside-the-map background, and the **fuel bar colours read as 15-bit RGB** (unverified format, document 70) are the port's; the original's frame, grid, bracket cursor and arrow are not drawn yet (document 69).
...
- **Level 1 accuracy, in order (agreed 2026-09-24):** (1) ~~trace the flag~~ **correction: the flag's pickup, carry, drop and the capture test are already traced (documents 57, 65; only a Jeep carries, and that is the original's rule, not a placeholder); what was untraced was the sequence after the win, now traced ([document 92](92-worked-example-what-happens-after-the-flag-comes-home.md): fade to black, the winner's ribbon bitmap from `TITLE\Ban*.BMP`, a victory jingle, then back to the front end) and the ribbon is in the port. Still open from it: the jingle (`Win*.stm`, an undecoded `auds` stream on the CD image), what the front end does next, and the flag's own leftovers (direction arrow, own-flag return at the base `FUN_00432600`, the flag sounds, the radar blip)**; (2) ~~the building's 62 -> 63 stage~~ **already traced and applied (document 44); the "not done" line in document 45 was stale; re-checked against `FUN_0042e8c0` in [document 93](93-worked-example-the-buildings-stages-checked-against-the-code.md): 6 plain hits per stage, no debris branch for these ids**; (3) ~~terrain passability and height~~ **traced ([document 94](94-worked-example-terrain-passability-and-the-jeeps-road-following.md)): the "elevation" bits are hit points and there is no terrain height; the speed factors, water classes and collision shapes were already in; the one missing piece, the Jeep's road-following steering, is now in the port**; (4) ~~vehicle loss and the life system~~ **traced ([document 95](95-worked-example-after-the-skull-the-choice-and-the-loss.md)): after the skull the vehicle choice opens (now in the port); the loss test counts Jeeps only (no Jeep in stock or alive = lost; the port used "all stock gone"); losing shows no message, only fades to black and leaves the level; there is no separate lives counter, the stock is the life system**; (5) the real panel (fuel and health displays, announcer): **its screen layout is now traced and built as an option ([document 96](96-worked-example-the-original-screen-layout-and-the-classic-hud.md)): `classic` = the original's (320 x 152 game view above the backdrop strip `ARTPBSCRL.RFA`, the panel at (87, 168)), `modern` = the port's earlier bottom-left overlay (the default for now); `user://settings.cfg` `[hud] layout`, `RF_HUD`, `H` in a debug build; the second screen-size layout and the two-player screens are not built**. Also possible: a frame-by-frame comparison of the port against the reference video.
- **Sound is unfinished (user, 2026-09-24):** (a) ~~the continuous sounds are missing~~ **the vehicles' engine loops are traced and built ([document 97](97-worked-example-the-engine-loops.md): Tread for Tank and MSV, JeepIdle, the Heli's rotor, pitched by speed or rotor speed; the mixer's volume scale, the loop window and other vehicles' positional falloff are untraced; the `Drone` sound belongs to an unidentified flying object, `FUN_0040a7b0`); other continuous sounds (water, gate, ...) are not looked for yet**; (b) ~~there is no music at all~~ **the music is traced and built ([document 98](98-worked-example-the-music.md)): the real soundtrack is on the CD image (`tools/extract_music.py`), the 18 "announcer voice lines" of document 71 are its music lines, and the director (priority queue, state machine, transitions) is reproduced in `game/music_director.gd`; volume, the fade length, the title screen's Drums, the Sub and Win lines, two-player rules and the source-object cut are open, and I have not listened to it**.
- **Classic HUD layout, to finish later (user, 2026-09-25; document 96):** `modern` stays the default. In `classic` the hangar screen, the loss skull, the victory ribbon and the fades are still drawn over the whole window instead of the 320 x 240 picture; the second screen-size layout (`DAT_0048c7dc != 0`: a 357 x 169 view, panel at (116, 185), selector untraced) and the two-player screens (two panels, the `2PBSCRL/H.RFA` strips) are not built; the panel's slide-in animation and the radar's grid, brackets and background (document 70) are not drawn in either layout; the classic view's world scale is the port's, not the original's coverage (needs the camera-unit mapping of document 90).
- **Moddability framework (user direction, 2026-09-24)** — one data-driven vehicle framework, per-map vehicle rosters with override files, split-up art; steps 1-4 (layered packs; sprite ids and loose-frame art; vehicle definitions; behaviour modules) done, steps 5-7 open — plan section 2.7; the editor itself is planned in [`EDITOR_PLAN.md`](../EDITOR_PLAN.md), phase E0 built (`editor/`).
...
- ~~**A life system the user also described — not yet investigated at all.**~~ **Resolved (2026-09-24, document 95): the stock is the life system; losing is "no Jeep left".** (Original note: No "Life"/"Lives"
...
- ~~**Terrain-based vehicle passability**~~ **Resolved (2026-09-24, document 94):** the elevation bits / height_seed byte are hit points (documents 44, 93), there is no terrain height, and what the terrain does to a vehicle (speed factors, water classes, collision shapes, and the Jeep's road-following steering) is traced and applied.
