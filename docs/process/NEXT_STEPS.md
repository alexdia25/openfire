...
- **Hatch doors: traced, they do not animate (document 89, 2026-09-24).** The pad is *not* two doors opening: on dock (`FUN_0042f110`) the pad tile's art is swapped for the fully transparent art 92 and stays a hole through the sink, the choice screen and the confirm script; on undock the lift object rises from -32 to 0 at 0.3 a tick (~107 ticks) out of the hole, and only then is the tile restored (`FUN_0042ec50`) and the real vehicle created. `FUN_004161e0`/`FUN_00416300` (the lead from document 81/65) are a camera rig, not a door. **The port does neither the tile swap nor the rise yet** (the docking vehicle sinks through an intact opaque hatch, and the undocked vehicle appears at full height); implementing both needs a per-tile art override in the terrain renderer and coordination with the vehicle-framework work on `match_controller.gd`. Also untraced: what a transparent tile shows in the original (presumably black), and what the two camera rigs do to the view during dock/undock.
