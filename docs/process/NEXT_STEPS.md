...
- **The MSV's mines and the explosion damage box (2026-09-21):** `M` lays a mine (every 140 ticks); it blinks, beeps faster as it ages, goes inert after ~158 ticks, and is set off by a moving vehicle. Its explosion owns a damage box that hurts vehicles and buildings for 2.4 s. Placeholder: the dropper is exempt until it has left the trigger box (the code has no such rule); the literal behaviour is `RF_DEBUG_MINE_LITERAL=1`. See [document 60](60-worked-example-mines.md).
