...
- **Debug vehicle swap (2026-09-21):** `F1` / `F2` / `F3` turn the player into a Tank / Jeep / MSV anywhere, at once, with fresh hit points, fuel and weapon state (a flag carried by a non-Jeep is dropped). Port-only, alongside the `V` home-tile switch and `RF_VEHICLE=tank|jeep|msv`.
