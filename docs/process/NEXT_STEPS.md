...
- **Sound is unfinished (user, 2026-09-24):** (a) ~~the continuous sounds are missing~~ **the vehicles' engine loops are traced and built ([document 97](97-worked-example-the-engine-loops.md): Tread for Tank and MSV, JeepIdle, the Heli's rotor, pitched by speed or rotor speed; the mixer's volume scale, the loop window and other vehicles' positional falloff are untraced; the `Drone` sound belongs to an unidentified flying object, `FUN_0040a7b0`); other continuous sounds (water, gate, ...) are not looked for yet**; (b) **there is no music at all**: the mechanism is traced ([document 14](14-worked-example-music-mechanism.md), CD audio with a WAV fallback) but nothing plays it. Both are unstarted.
