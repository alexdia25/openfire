...
**A concrete, data-backed lead for the Heli's "different textures in full flight" report:** document 38's registry audit already extracted real 3D box geometry for the Heli (12 parts, cels 524-574, `tools/data/vehicle_type_parts.json` key `"3"`) the same way it did for the Tank before `game/vehicle_box_3d.gd` was built (documents 37-40) — but nothing renders it. The Heli still uses the old flat billboard/ground-decal path (`game/vehicle_billboard_3d.gd`), the exact class of simplification that caused the Tank's own "little triangles" complaint before real geometry fixed it (document 32). That alone is a plausible explanation for "different textures in full flight": a real 3D shape exposes different faces as it banks/climbs; a flat decal never does.
