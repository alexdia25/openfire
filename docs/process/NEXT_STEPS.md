...
- **The hatch DOES open (document 89, 2026-09-24; the first version of that document wrongly said it did not, the user was right).** The pad is a *pit model* drawn by the lift objects (walls cels 826-828, plate 829/830, hazard strip 831) with a **two-leaf lid**: cels 822-825 (the registry had them as `rescue_cross`, now `structure.hangar_leaf.*`) drawn `0.3 * age + 5` units to each side and hidden at age 60. On undock the leaves start closed and slide apart over 60 ticks while the vehicle rises from -32 to 0 at 0.3 a tick (~107 ticks); on dock (read from code only, no docking in the footage) the 70-tick timer counts down, so the leaves are retracted for 10 ticks and then slide shut over the sinking vehicle. The pad tile is swapped to transparent art 92 for the whole time and restored at the end (`FUN_0042ec50`). `FUN_004161e0`/`00416300` are a **camera rig**, and the footage shows it as a swoop-in from a small distant view when the game view opens. **The port has none of this yet** (no tile swap, pit, leaves, rise, or swoop-in): see the "Not done" list of document 89. Untraced: the swoop-in's exact curve, where the hazard border around the open pit comes from, the leaf shift's frame of reference.
