...
- **A prototype confirming the "little triangles" complaint is a rendering-technique gap, not missing art — and that combining it with more real art actually makes it worse.** A real, non-billboarded quad lying flat like the terrain plane, rotated in true 3D to match the vehicle's actual heading, produces a smooth, always-coherent silhouette at every heading using just **one** source image — better than the old nine-frame mirror-and-flip approach ever achieved. Tested the obvious follow-up (cycling all 9 real frames alongside the same rotation) and it's worse, not better: the discrete frames already bake in a thinning silhouette near 90/270 degrees, and real geometric foreshortening on top compounds that instead of complementing it, collapsing the shape almost to nothing at exactly those headings. Toggles on `game/vehicle_billboard_3d.gd`: `RF_DEBUG_VEHICLE_QUAD_MODE=ground_decal` (single texture, wins) or `=ground_decal_multi` (9 textures, loses); default stays the verified Phase 3 billboard. See [document 32](32-worked-example-ground-decal-prototype.md); plan section 2.2, section 4 item 10.
