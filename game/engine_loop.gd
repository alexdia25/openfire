...
## The volume is the descriptor's level (document 100: see SoundLevel); the bytes document 97 took for a volume envelope (210, then 100 after 10 ticks) are the voice's priority.
...
## The loop's volume in dB (constant while it plays).
static func volume_db(l: Dictionary) -> float:
	return SoundLevel.db(float(l["level"]))
