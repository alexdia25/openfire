...
func setup(controller: MatchController) -> void:
	...
	_glow.draw.connect(func():
		if mc.selecting:
			var at := _atlas(int(_sel["cels"]["highlight"]))
			if at.atlas != null:
...
func _atlas(cel: int) -> AtlasTexture:
	if _tex_cache.has(cel):
		return _tex_cache[cel]
	var id: String = _sel["sprite_ids"].get(str(cel), "")
...
func _draw() -> void:
	...
	for t in 4:
		...
		var cel_pic := int(cels["picture_base"]) + t * 2 + team
	...
	var body := _atlas(int(cels["platform_body"]))
	if body.atlas != null:
	...
	_blit(int(cels["platform_cap"]), Vector2(px, py))
...
func _draw_backdrop(team: int, screen: Vector2) -> void:
	...
	var cels: Dictionary = _sel["cels"]
	for b in _backdrop:
		_blit(int(b[0]), Vector2(b[1], b[2]))


## FUN_00417500: the cel list of the backdrop for `team`, using the C runtime's rand (state * 214013 + 2531011, bits 16-30) as the game does.
func _build_backdrop(team: int, screen: Vector2) -> void:
	...
	var cx := screen.x / 2.0
	var strip_w := 64.0
	var centre := int(cels["strip_centre"])
	_backdrop.append([centre, cx - 32.0, 0.0])
	_backdrop.append([centre, cx - 32.0, 16.0])
	var x := cx - 32.0
	while x >= 0.0:   # left of the centre strip: 64, 0, -64
		x -= strip_w
		_backdrop.append([int(cels["strip"][0]) + rng.below(2), x, 0.0])
		_backdrop.append([int(cels["strip"][0]) + rng.below(2), x, 16.0])
	x = cx - 32.0 + strip_w
	while x < screen.x:
		_backdrop.append([int(cels["strip"][0]) + rng.below(2), x, 0.0])
		_backdrop.append([int(cels["strip"][0]) + rng.below(2), x, 16.0])
	...
	while x < screen.x:   # clouds along the horizon
		_backdrop.append([int(cels["cloud"]) + rng.below(3), x, float(_sel["backdrop"]["cloud_y"])])
		x += 64.0
	x = 0.0
	while x < screen.x:   # dirt, each column reseeded with the player and its x in 16.16
		...
		while y < screen.y:
			_backdrop.append([int(cels["dirt"]) + rng.below(4), x, y])
			y += 32.0
		x += 32.0
...
func _draw_panel() -> void:
	...
	_blit(int(cels["panel_frame"]), pp + Vector2(-3, -2))
	_blit(int(cels["panel_interior"]), pp + Vector2(9, 2))
	var t := mc.selection
	_blit(int(cels["icon"][t]), pp + off + Vector2(6, 1))
	_draw_number(mc.vehicle_stock[t], pp + off, 0x1D, 3)
	_blit(int(cels["weapon_icon"][t]), pp + off + Vector2(6, 0x1A))
	_draw_number(int(_sel["ammo_capacity"][t]), pp + off, 0x11, 0x1C)
	var second := int(cels["second_icon"][t])
	if second != 0:
...
func _draw_number(value: int, origin: Vector2, x: int, y: int) -> void:
	...
	var digit0 := int(_sel["cels"]["digit_base"])
	var cx := x
	if v > 99:
		_blit(digit0 + 1, origin + Vector2(cx, y))
		cx += 6
	if v > 9:
		var tens := (v / 10) % 10
		_blit(digit0 + tens, origin + Vector2(cx, y))
		cx += 8 - (2 if tens == 1 else 0)
	_blit(digit0 + v % 10, origin + Vector2(cx, y))


func _draw_map(screen: Vector2) -> void:
	var fpos := Vector2((screen.x - 144.0) / 2.0, (screen.y - 144.0) / 2.0)
	_blit(int(_sel["cels"]["map_frame"]), fpos)
	if _map_image != null:
