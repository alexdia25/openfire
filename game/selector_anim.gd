...
## step list at `state +0xc8` (FUN_00418040 calls each step's routine every tick until it answers "done"). Sound steps are skipped for now (the sound pass).
...
func step(ticks: float) -> void:
	...
	while _i < _steps.size():
		...
		match String(s[0]):
			"sound":
				done = true   # PLACEHOLDER until the sound pass (document 78)
			"platform_up":
				d0 -= float(s[1]) * ticks
				if d0 <= float(s[2]):
					d0 = float(s[2])
